...
class contentsearchm extends CI_Model
{
	...
	function englishreadycontentm()
		...
		$this->db->select('masterproducttable.mpt_id,masterproducttable.prduct_name, masterproducttable.product_sku,masterproducttable.product_upc, masterproducttable.product_brand,masterproducttable.product_source,masterproducttable.status');
		$this->db->from('masterproducttable');
		$this->db->from('finalproductlist');
...
//		$this->db->limit(1);
	...
	function englishready(){
		...
		$finaproductdata = array (
					...
					'product_disclaimer'=>htmlspecialchars($_POST['pDisclaimer']),
	...
	if($updatedfinal){
		  ...
		  $spanishproductdata = array (
					...
					'product_disclaimer'=>htmlspecialchars($this->splitstring($_POST['pDisclaimer'])),
					'isset'=>htmlspecialchars($_POST['pisset']),
					'attributes'=>htmlspecialchars($att),
					'onlineonly'=>htmlspecialchars($_POST['ponlineonly'])
								  );
		...
		while($r = mysql_fetch_array($re)){
			echo $q = "UPDATE `masterproducttable` SET `status` = '7',`fpl_id` = $englishinsert_id,`sppr_id` = $spanishinsert_id WHERE `mpt_id` =".$r['mpt_id']."AND `product_source` =".$_POST['pSource'];
			mysql_query($q);
		}
