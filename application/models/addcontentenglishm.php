<?php
class addcontentenglishm extends CI_Model
{
	...
	function get_brand_dropdowns($selectval)
	{
	$this->db->select("*");
	$this->db->where('status',1);
	$this->db->from('brandmanagement');
	...
	if($data->num_rows() > 0)
	{
	$return="<select name='pBrand' id='pBrand' onchange='savebrandname(this);'><option value=''>Please select brand</option>";
	$value=$data->result();
	foreach($value as $values)
	{
	if($selectval == $values->bMagentoId)
	$selection = 'selected=selected';
	else
	$selection = "";
	$return.="<option $selection value=".$values->bMagentoId.">";
	$return.=$values->brandName;
	$return.="</option>";
	}
	$data->row();
...
//		$this->db->limit(1);
	...
	$collectcat = "";
		...
		$this->update_master_status($fpl_id);
	...
	function update_master_status($fpl_id)
	{
		$data = array (
				'status' =>14
						);
