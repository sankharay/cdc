<?php
class directmagentom extends CI_Model
{
	...
	function senddatatodb($file,$vendortemplatedetails,$vendorid,$attributes)
	{
	   ...
	   if(in_array($data,$vendortemplatearray))
	   {
		...
		if($attributes != '')
		{
			...
			{
			// echo "<br>".$excel->sheets[0]['cells'][$i][$attributesa[$a]]." --------- ";
				$realsdata = $excel->sheets[0]['cells'][$i][$attributesa[$a]];
				$attributesss="";
				$davalue = $excel->sheets[0]['cells'][$i][$attributesa[$a]];
				$subattributesaa = explode('|',$davalue);
				$subattributesasizea = sizeof($subattributesaa);
				for($e=0;$e<$subattributesasizea;$e++)
				{
				$subattributevalues = $excel->sheets[0]['cells'][$i][$attributesa[$e]];
				$attributesss.= $subattributevalues.",";
				}
				$dbdataarray['attributes'].= $attributesss.",";
			}
			}
			...
			{
			$dbdataarray['status']=1;
			$this->directmagentotable($dbdataarray);
			// end copy images
			}
			else
			{
	...
	{
			$this->db->insert('directmagentotable_duplicates',$data);
			return $this->db->insert_id();
	}
	...
	function directmagentotable($data)
	{
			$this->db->insert('directmagentotable',$data);
			return $this->db->insert_id();
	}
	
	function check_upc_vendor($upc,$vendorid,$dbdataarray)
	{
	$this->db->select('product_upc,fpl_id');
	$this->db->where('product_upc',$upc);
	$this->db->from('directmagentotable');
	$data = $this->db->get();
	if($data->num_rows() > 0 )
	{
		return TRUE;
