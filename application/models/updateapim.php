<?php
class updateapim extends CI_Model
{
	...
	function matchexcelfields($file,$vendortemplatedetails)
	{
		...
		{
		$dataa = trim($excel->sheets[0]['cells'][3][$i]);
	...
	function senddatatodb($file,$vendortemplatedetails,$vendorid,$templateid)
	{
	   ...
	   if(in_array($data,$vendortemplatearray))
	   {
		  ...
		  $columns[] =  $i;
			...
			print_r($dbdataarray);
			echo "<br></pre>";
			$upc_exist = $this->check_upc_vendor($currentsku,$vendorid);
			if($upc_exist == FALSE)
			{
	...
	function check_upc_vendor($sku,$vendorid)
	{
