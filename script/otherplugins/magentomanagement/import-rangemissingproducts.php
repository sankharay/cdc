<?php
	...
	Mage::app('default'); 
        $currentStore = Mage::app()->setCurrentStore(1);
			...
			$content = get_missing_data();
			while($contents = mysql_fetch_object($content))
			{
...
echo "SELECT * FROM `finalproductlist` WHERE `magento_category_id`='$id'";
			...
			$productname = htmlspecialchars($getName);
			 $short_description = str_replace($search, $values, $product->getShortDescription());
			$productescription = htmlspecialchars($short_description);
			 $description = str_replace($search, $values, $product->getDescription());
			$productgetDescription = htmlspecialchars($description);
			...
			$productgetweight = $product->getweight();
			 $getmeta_keyword = str_replace($search, $values, $product->getmeta_keyword());
			$productgetmeta_keyword = htmlspecialchars($getmeta_keyword);
			 $getmeta_description = str_replace($search, $values, $product->getmeta_description());
			$productgetmeta_description = htmlspecialchars($getmeta_description);
