<?php
	...
	Mage::app()->setCurrentStore(Mage_Core_Model_App::ADMIN_STORE_ID);
			...
			if(($que % 100) == 0)
			sleep(120);
			$checskuinmagento = checkskuinmagento(trim($content->sku));
	if($checskuinmagento == TRUE )
	echo "there";
	else
	echo "not there";
					...
					$product_id = Mage::getModel('catalog/product')->getIdBySku(trim($content->sku));
								...
								// inventry data update start
										if($content->vendorid != "" AND $content->vendorid != "~")
										{
										$getvendorid = $product->getvendorid();
											if($getvendorid == $content->vendorid)
											{
...
echo "-".$content->sku."-";
					...
					{
						$dataprocessed = updatedatainventryids($content->sku);
