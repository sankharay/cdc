...
$j=0;
	...
	Mage::app()->setCurrentStore(Mage_Core_Model_App::ADMIN_STORE_ID);
		...
		if($result = $mysqli->query("select * from direct_watch_data_duplicate where status=1"))
		{
		...
		while($content = $result->fetch_object())
		{
				...
				$noimageentry = $mysqli->query("UPDATE `direct_watch_data_duplicate` SET `imagestatus`='1' WHERE `SKU`='$sku'");
				}
				else
				{
				$from = trim($img[$i]);
				$urlimg = rand(2001,10000);
				$targetimagename = str_replace('/','_',$sku).'_'.$urlimg.'.jpg';
				$image = IMAGES_MAGENTO_LOCATION_URL.$targetimagename;
				if(!@copy($from,$image))
				{
...
;
				...
				try {
				$product->save();
				$imageentry = $mysqli->query("UPDATE `direct_watch_data_duplicate` SET `status`='2' WHERE `SKU`='$sku'");
				...
				{
				$imageentry = $mysqli->query("UPDATE `direct_watch_data` SET `status`='8' WHERE `SKU`='$sku'");
